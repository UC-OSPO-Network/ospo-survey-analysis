...
Request from a peer reviewer: how many people said Non-applicable for teaching and research?
```{r}
# Teachers
total = nrow(importance_and_job)
tea_nas = sum(importance_and_job$Teaching == "Non-applicable")
applicable = total - tea_nas
sprintf("total respondents: %d", total)
sprintf("number of NA responses for teaching: %d", tea_nas)
sprintf("number of teachers, based on applicable/NA response: %d", applicable)
sprintf("percent of respondents who are teachers: %.2f%%", (applicable / total)*100)

# Researchers
total = nrow(importance_and_job)
res_nas = sum(importance_and_job$Research == "Non-applicable")
applicable = total - res_nas
sprintf("total respondents: %d", total)
sprintf("number of NA responses for research: %d", res_nas)
sprintf("number of researchers, based on applicable/NA response: %d", applicable)
sprintf("percent of respondents who are researchers: %.2f%%", (applicable / total)*100)
```


Across the board, how many respondents said OS was important for their learning and/or PD?

```{r}
learning_global_vi <- nrow(importance_and_job %>% filter(Learning == "Very important"))
pd_global_vi <- nrow(importance_and_job %>% filter(`Professional Development` == "Very important"))

learning_global_vi/nrow(importance_and_job)*100
pd_global_vi/nrow(importance_and_job)*100
```

186/294 and 187/294, or about 63%.

How about the same statistics, but this time counting "more than moderately important".

```{r}
learning_global_mmi <- nrow(
  importance_and_job %>%
    filter(Learning == "Very important" | Learning == "Important")
)
...
learning_global_mmi/nrow(importance_and_job)*100
pd_global_mmi/nrow(importance_and_job)*100
```

Sanity check: what's the "venn diagram" of people who answered for the "job" category but aren't NR staff?

```{r}
nrow(importance_and_job %>% filter(job_category == "Non-research Staff"))
nrow(importance_and_job %>% filter(job_category == "Non-research Staff" & Job != "Non-applicable"))
nrow(importance_and_job %>% filter(job_category != "Non-research Staff" & Job != "Non-applicable"))

```

Okay, so 104 out of 106 nr staff answered that question. 83 people who are not nr staff also answered that Q. I will discard those 83, since that question was never intended for them, based on pre-test feedback that it's hard for researchers to know what work counts as "job" but not "research".
...
write_df_to_file(
  more_than_mod,
  file.path("data_for_plots/importance.tsv")
)
```
...
This is the version I've used for powerpoint presentations
```{r}
save_plot("importance_all_pct.tiff", 12, 8, p=p)
```

This is the version for the journal, meeting their submission requirements.
```{r fig.width=16, fig.height=8}
p2 <- ggplot(
  more_than_mod,
  aes(
...
ggsave(
  file.path(FIGURE_PATH, "fig3.tif"), p2,
  width  = 5.2,
  height = 5.2 * (8/12),   # = 3.4667 in
  units = "in",
  dpi = 600
)
```
