...
# Overview
This script creates bar plots from question 2 on the survey, which is about the
perceived importance of open source for different job categories and different
tasks.
...
get_percent_more_than_moderate <- function(df) {
  ...
  n_high_rows <- nrow(df$importance_level %in% high_importance_cats)
  n_low_rows <- nrow(df$importance_level %in% low_importance_cats)
  total <- nrow(df)
  if (n_high_rows + n_low_rows != total) {
    stop("`df` has at least one unexpected value in 'importance_level'.")
  }
  pct <- round(n_high_rows / total, 2)
  return(pct)
}
```
...
Finally, let's check what percentage of teachers said OS is more than 
moderately important for their teaching.
```{r}
teaching_high <- teaching %>%
  filter(
    importance_level == "Very important" |
      importance_level == "Important"
  )

teaching_low <- teaching %>%
  filter(
    importance_level == "Moderately important" |
      importance_level == "Slightly important" |
      importance_level == "Not at all important"
  )
nrow(teaching_high)
nrow(teaching_low)
nrow(teaching)
round(nrow(teaching_high) / nrow(teaching) * 100, 2)
```
...
Finally, let's check what percentage of researchers said OS is more than 
moderately important for their research.
```{r}
research <- research_learning_pd %>%
  filter(
    importance_area == "Research"
  )
research_high <- research %>%
  filter(
    importance_level == "Very important" | 
    importance_level == "Important"
  )
