...
```{r}
overall_plot <- basic_bar_chart(motivations_to_plot,
  x_var = "Motivation",
  y_var = "Count",
...
Create a plot where all jobs are scaled to 1.
```{r}
...
Visualize
```{r}
stacked_plot_raw_it + stacked_plot_proportional_it
```

Save
```{r}
save_plot("motivations_stacks_it_academics.tiff", 12, 8)
```
