...
# Minimal model (no job) (added by peer reviewer request)

```{r}
fit0 <- ordinal::clmm(utility ~ solution +      
              (1 | participantID),                 
...
## "Global" emms: averaging over job category
First, let's explore the "overall" ratings with different weighting schemes. I'm not cherry picking here, I'm just trying to understand the options.
...
We only get two sets of estimates: equal/flat gives us the estimates where all means are given equal weight. Prop, outer, and cells give us another set of estimates, where each prediction is given the weights proportional to sample size. At least, I think that's how it works. 

We know from the descriptive statistics and exploratory plots that "a learning community" is more popular among non-research staff than among other groups. So, we expect that if all groups are weighted equally, "a learning community" will be less popular than if we weight the means by sample size.

Hmm. When I try running the command below, to get global solution ratings averaged over job, the command fails when I include mode="mean.class". It says no weighting information is available.
```{r}
summary(emmeans(fit1b, ~ solution, weights = "equal", mode = "mean.class")) %>% 
  arrange(desc(mean.class))
```

UPDATE: This was indeed a bug that will be fixed in the next version of emmeans. https://github.com/rvlenth/emmeans/issues/553

Below is Russ Lenth's suggested workaround to "manually" calculate weighted means on the mean.class scale.

```{r}
emm.pr <- emmeans(fit1b, ~ utility|solution, weights = "prop", mode = "prob")
emm.pr.res <- contrast(emm.pr, list(mc = 1:3)) |> confint(by=NULL) |> arrange(desc(estimate))
```

```{r}
write_df_to_file(emm.pr.res, "supplementary_tables/solns_global_mean_ratings_prop_weighing.tsv")
```

```{r}
emm.eq <- emmeans(fit1b, ~ utility|solution, weights = "equal", mode = "prob")
emm.eq.res <- contrast(emm.eq, list(mc = 1:3)) |> confint(by=NULL) |> arrange(desc(estimate))
emm.eq.res
```

```{r}
# From utils.R
write_df_to_file(emm.eq.res, "supplementary_tables/solns_global_mean_ratings.tsv")
```

When we use the default weighting of "equal", "A learning community" is #8, but with "prop" weighting, it rises to #5. This makes sense, as discussed above.

It's not great to average over a factor that we've already established is important. I'll report it, but with big caveats in the text. I like prop weighting since this is an observational study--the samples are not equal. For lack of a better idea (e.g. weighting groups by expected population size), we are "correcting" for the differences in sample size, so that all groups get equal weight.

## Emms for each solution, for each job category
Here, we get the mean ratings for each job, ordered by job. 
```{r}
all_means <- emmeans(fit1b, ~ solution | job_category, mode="mean.class")
summary(all_means) %>%
 arrange(desc(mean.class))
```

The same data, but ordered by solution.
```{r}
all_means2 <- emmeans(fit1b, ~ job_category | solution, mode="mean.class")
summary(all_means2) %>%
 arrange(desc(mean.class))
```

## Pairwise comparisons
Here, we are looking at all possible pairwise comparisons. Again, we are no longer averaging over job category.

I'm using Russ's workaround again because I was playing around with different weighting schemes. In the end, I decided on equal, the default.

```{r}
emm.eq.job <- emmeans(fit1b, ~ utility | job_category * solution, mode = "prob")
emm.eq.job.res <- contrast(emm.eq.job, list(mc=1:3))
emm.eq.job.res |> confint(by=NULL)
```

Here, we get all pairwise comparisons, with p-values. The "estimate" here is the difference between the estimated means above.
```{r}
by_job <- summary(
  pairs(emm.eq.job.res, by = "job_category"),
...
```{r}
# From utils.R
write_df_to_file(by_job, "supplementary_tables/solutions_pairwise_contrasts_byjob.tsv")
```

And do the same, but sorting by solution.
```{r}
by_soln <- summary(
  pairs(emm.eq.job.res, by = "solution"),
  infer = TRUE # infer CIs
)
by_soln
```
...
```{r}
# From utils.R
write_df_to_file(by_soln, "supplementary_tables/solutions_pairwise_contrasts_bysoln.tsv")
```
...
Hm. The results are a little surprising. Only five solutions are "divisive" according to this test. But maybe it's not surprising, if the whole point of doing ordinal regression is that it's more sensitive than a non-parametric test.
...
make_plot <- function(df, jc) {
  ggplot(filter(df, job_category == jc),
    ...
    geom_point(size = 3) +
    ylim(c(1, 3)) +
    labs(title = jc, x = NULL, y = "Estimated mean rating (0–3)") +
    coord_flip() +                       # solutions run down the y‑axis
    theme(
...
```{r}
write_df_to_file(data.frame(soln = solns_ordered), "data_for_plots/solns_ordered.tsv")
write_df_to_file(emm_clean, "data_for_plots/emms_solns.tsv")
```
