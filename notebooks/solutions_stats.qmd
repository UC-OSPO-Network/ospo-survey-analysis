...
TODO: CHANGE TO THIS: emmeans(fit1b, ~ job_category * solution, mode = "mean.class")
```{r}
...
This looks reasonably consistent with the model. It appears that even though "A learning community" was the number 1 solution from non-research staff when we asked them to choose their top solution (see solutions_plots.qmd), when they were allowed to rate solutions on a Likert scale, sustainability grants were a bit more popular.
...
```{r}
global_soln_ratings <- emmeans(fit1b, ~ solution, mode = "mean.class")
# equal weights because we're just looking at solutions
```
We get a warning, but this makes sense. Of course, when we are asking for a global mean, we are ignoring the effects of job.
