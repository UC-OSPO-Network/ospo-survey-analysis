...
I saved my work in an excel spreadsheet. The most important column is the "decision" column, so I just copied that one column to a code-friendly text editor and saved it as a tsv. Here's a key to my mark-up in that column:
k = keep as-is
d = drop
...
Ok! Now we finally have the qualitative responses smooshed and zhoozhed into our taxonomy. Let's save this to a file.

```{r}
write.table(
  curated,
  file.path(Sys.getenv("DATA_PATH"), "qual_disciplines_final.tsv"),
  row.names = FALSE,
  quote = FALSE,
  sep = "\t"
)
...
write.table(
  final_data,
  file.path(Sys.getenv("FIGURE_PATH"), "qual_disciplines.tsv"),
  row.names = FALSE,
  quote = FALSE,
  sep = "\t"
)
