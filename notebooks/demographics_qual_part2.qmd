...
I saved my work in an excel spreadsheet. The most important column was the "decision" column, so I just copied that one column to a code-friendly text editor and saved it as a tsv. Here's a key to my mark-up in that column:
k = keep as-is
d = drop
If I put a number in the decision column, that means replace the existing row with that row of the taxonomy.
Also, the taxonomy didn't have "AI" or "Machine Learning" by themselves. So I tagged people who wrote in "AI" or "ML" as 1017, "Artificial Intelligence and Robotics", even though none of these people mentioned robotics. I'll correct that later in this script.
...
```{r}
auto_results <- load_qualtrics_data("curation_of_disciplines/qual_fields_guesses.tsv") 

# a manually generated one-column table
# do not read with my load_qualtrics_data function bc it has header = TRUE
decisions <- read.csv(
  file = file.path(DATA_PATH, "curation_of_disciplines/classification_decisions.tsv"),
  header = FALSE,
  sep = "\t",
  check.names = FALSE,
  stringsAsFactors = FALSE
...
Change "Artificial Intelligence and Robotics" to "AI and Machine Learning". We gave pure "Robotics" to those answers that mentioned it. Nobody actually entered AI AND robotics, so all occurences of this are just AI.
```{r}
curated$Level3[
  curated$Level3 == "Artificial Intelligence and Robotics"
...
```{r}
write_df_to_file(curated, "curation_of_disciplines/qual_disciplines_final.tsv")
```
...
I'm just going to leave them as a table for now. 

```{r}
write_df_to_file(final_data, "curation_of_disciplines/qual_disciplines_summary.tsv")
```
