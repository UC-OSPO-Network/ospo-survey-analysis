...
- Arguments:
  - `df`: A data frame where rows are participants, and columns are `Role`
    plus at least one response variable, e.g. `Skills`, `Give back`, etc.
    Extra columns are okay. The response variable of interest should be a
    column of 0s and 1s.
...
The distribution of the residuals seems normal-ish. Note I'm just picking one of the categories that had a weird coefficient, to make sure the data in that category aren't too weird.
```{r}
...
```{r}
# Exclude non-research staff
academics <- motivations_processed %>% 
  filter(
    Role == "Faculty" |
      Role == "Grad Students" |
...
We can also look at the Pr(>Dev) for each univariate test, which I'm assuming are p-values. These show that the only motivation that is predicted on its own by these two groups is "Job", with a p-value of 0.023.
...
```
Uh-oh. The pairwise comparison fails. Well, that makes sense. The two groups must be different from each other, or else the global model fit wouldn't be significant. So we actually don't need this test.
...
```{r}
to_plot <- data.frame(proportion_yes=c(p_acad_yes*100, p_it_yes*100), role=c("Academic", "IT"))
...
```{r}
save_plot("acad_it_simple.tiff", 8, 9, p=myplot)
```