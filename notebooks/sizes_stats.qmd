...
Before considering job categories or doing regression, let's just look at the percentages for different size and frequency levels.
```{r}
combined2 %>%
...
## Model 1.1: no job data + random effect (This section was added later based on a peer reviewer's comments)
Let's spend a little more time looking at the dataset without consideration for job categories. As above, it doesn't matter which data frame we use since we're ignoring the job category column.
```{r}
fit1.1 <- ordinal::clmm(
  frequency ~ size + (1 | participantID),
  data = combined4,
  link = "logit",
  Hess = TRUE
)
...
Let's exponentiate the coefficients to get odds ratios, and subtract from 100 to get percentage changes between size categories.

```{r}
100-exp(fit1.1$coefficients["sizeMedium"])*100
100-exp(fit1.1$coefficients["sizeLarge"])*100
```

And check whether we can use the proportional odds assumption, at least for the equivalent non-mixed model. 
```{r}
nominal_test(fit1)
```
...
```{r}
# From utils.R
write_df_to_file(res, "supplementary_tables/projsizes_means.tsv")
```
...
```{r}
# From utils.R
write_df_to_file(by_job, "supplementary_tables/projsizes_by_job.tsv")
```
...
```{r}
# From utils.R
write_df_to_file(by_size, "supplementary_tables/projsizes_by_size.tsv")
```
