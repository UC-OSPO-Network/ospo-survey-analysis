...
For a little confidence boost, let's try collapsing the frequency levels into a binary of Never vs. Some frequency, and then do a logistic regression. We'll need to use glmer instead of glm since we have a mixed model, which glm doesn't support.

If this regression also says that all groups are more likely to contribute to small projects than large projects, then we have more confidence in that conclusion.

```{r}
combined4_2 <- within(combined4, some <- frequency != "Never")

fit_bin <- lme4::glmer(
  some ~ job_category * size + (1 | participantID),
  family = binomial,
  data = combined4_2
)
```

Bah! Fails to converge

```{r}
combined2_2 <- within(combined2, some <- frequency != "Never")
...
Okay, had to switch to NR staff vs. academics to get the model to converge. I'm fine with this, since it's literally the same research question, conceptually, and just a different model, which is the whole point here.

```{r}
emm_bin <- emmeans(fit_bin, ~ size * job_category, mode = "mean.class")
summary(
  pairs(emm_bin, by = "job_category"),
  infer = TRUE # infer CIs
)
summary(
  pairs(emm_bin, by = "size"),
  infer = TRUE # infer CIs
)
```

Bah! Ugh, this regression says that NR staff are not significantly more likely to contribute to small projects than large projects.