...
```{r}
# I have no idea why this is necessary just for these plots...
# but it works
stack <- stack + theme(legend.key.size = unit(rel(1.5), "lines"))
stack_prop <- stack_prop + theme(legend.key.size = unit(rel(1.5), "lines"))

p_combined <- patchwork::wrap_plots(stack, plot_spacer(), stack_prop) +
...
svglite::svglite(
  file.path(FIGURE_PATH, "figureS1.svg"),
  width = 26,
  height = 10
)
print(p_combined)
