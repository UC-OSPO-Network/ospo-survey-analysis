...
Save the data frame for tweaking the final figure
```{r}
write_df_to_file(hosting_platform_data, "data_for_plots/vc_platforms.tsv")
```
...
```{r, fig.width=10, fig.height=7}
# Use a different color palette just to
# make it visually different from the other plots
field_colors = c(
    '#1a7937', #dark green
    '#acd49f', #light green
    '#BBBBBB', #gray
...
vc_hosting_field_plot <- ggplot(
  ...
  geom_bar(stat = "identity", position = "dodge") +
  ggtitle("Usage of hosting services by field") +
  labs(y = "Percent of respondents\nin field") +
  scale_fill_manual(values = field_colors) +
  scale_y_continuous(labels = scales::percent) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_text(size = 24, margin = margin(l = 20, r = 10)),
    axis.text.x = element_text(angle = 60, vjust = 0.6, size = 18),
    axis.text.y = element_text(size = 18),
    axis.ticks.x = element_blank(),
    legend.title = element_blank(),
...
Save the data frame for tweaking the final figure
```{r}
write_df_to_file(expanded_data_repos, "data_for_plots/data_repo_platforms.tsv")
```
...
data_repo_campus_plot <- ggplot(
  ...
  geom_bar(stat = "identity", position = "dodge") +
  ggtitle("Usage of data repositories by campus") +
  labs(y = "Percent of campus\nrespondents") +
  scale_fill_manual(values = COLORS) +
  scale_y_continuous(labels = scales::percent) +
  theme(
...
```{r}
svglite::svglite(file.path(FIGURE_PATH, "platforms_supp.svg"), width = 20, height = 12); print(p_combined); dev.off()
```
