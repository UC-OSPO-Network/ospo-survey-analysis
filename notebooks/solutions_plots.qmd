...
# Combine students into one group for visual clarity
job_fave <- job_fave %>%
  mutate(
    job_category = recode(
      job_category,
...
solutions_palette <- list(
  # modified from https://sronpersonalpages.nl/~pault/
  "Sustainability grants" = "#332288",
  "Computing environments" = "#77aadd",
  "Documentation help" = "#44bb99",
  "A learning community" = "#bbcc33",
  "Help finding funding" = "#eedd88",
  "Mentoring programs" = "#ee8866",
  "Education" = "#ffaabb",
  "Industry partnerships" = "#e5e5e5",
  "Legal support" = "#777777",
  "Containerization" = "#9B62A7",
  "Event planning" = "#46353A"
)
```

```{r, fig.width=9, fig.height=6}
q11_plot <- grouped_bar_chart(
  df = job_fave_top,
  x_var = "job_category",
  fill_var = "favorite_solution",
  title = "Top 5 Solutions by Job Category\n(Q11: 'choose one')",
  color_palette = solutions_palette
)
q11_plot
```

```{r}
save_plot("top_solutions_by_job.tiff", 12, 10, p=q11_plot)
```
...
```{r}
# For the sake of time, I am just hard-coding the colors
# in the proper order to match the other plots
solutions_palette2 <- list(
  # modified from https://sronpersonalpages.nl/~pault/
  "Sustainability grants" = "#332288",
  "Help finding funding" = "#eedd88",
  "Computing environments" = "#77aadd",
...
top_points_plot <- ggplot(
  ...
  scale_fill_manual(values = solutions_palette2) +
  labs(
    x = "Job Category",
...
```{r}
svglite::svglite(file.path(FIGURE_PATH, "figureS6.svg"), width = 20, height = 14); print(p_combined); dev.off()
```
