...
# Overview
This script creates donut plots that show survey participation rates across various groups.
If the script is run without arguments, it will create donut charts for all participants.
...
Here, I use a boolean variable called `contributor_mode` to record whether the user wants to create donut plots for all respondents or contributors only.
It's FALSE by default, so if you want to run this script interactively in a notebook, you can safely run this cell without any adjustments.
```{r}
...
Here's a look at the `job_data` data frame that will be used for plotting the donut chart.
```{r}
job_data
```
...
Plot #3 is of their field of study. Qualitative answers are not included in this plot.
```{r}
